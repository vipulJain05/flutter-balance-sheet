 ...
 */
  Future delete(String page, var docId, var olddate, var amount) async {
    ...
    if (page == "Revenue") {
      revenue = value == null
          ? 0
          : int.parse(value.documents[0].data['Revenue'].toString()) -
              int.parse(amount.toString());
      expense = value.documents[0].data['Expense'];
    } else {
      expense = value == null
          ? 0
          : int.parse(value.documents[0].data['Expense'].toString()) -
              int.parse(amount.toString());
    ...
    //Start Upload data Year Wise 
    //fetch data from server for data availability
    await Firestore.instance
        .collection(_email)
        .document('data')
        .collection('MonthWise')
    ...
    if (page == "Revenue") {
      _totalrevenue =
          int.parse(_yearValue.documents[0].data['TotalRevenue'].toString()) -
              int.parse(amount.toString());
      _totalexpense =
          int.parse(_yearValue.documents[0].data['TotalExpense'].toString());
    } else {
      _totalexpense =
          int.parse(_yearValue.documents[0].data['TotalExpense'].toString()) -
              int.parse(amount.toString());
    ...
    Map<String, dynamic> _yearRecord = {
      'Year': year,
      'Total': total,
      ...
      if (page == "REVENUE") {
        revenue = value == null
            ...
            : int.parse(data['amount'].toString()) +
                int.parse(value.documents[0].data['Revenue'].toString());
        expense = int.parse(value.documents[0].data['Expense'].toString());
      } else {
        expense = value == null
            ? 0
            : int.parse(data['amount'].toString()) +
                int.parse(value.documents[0].data['Expense'].toString());
        revenue = int.parse(value.documents[0].data['Revenue'].toString());
      }
      _monthTotal = revenue - expense;
